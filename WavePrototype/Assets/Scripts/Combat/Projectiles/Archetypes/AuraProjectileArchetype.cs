...
{
    public sealed class AuraProjectileArchetype : IProjectileArchetype
    {
        private readonly Collider[] _colliders;

        public AuraProjectileArchetype(int hitBufferSize)
        {
        ...
        public void Simulate(ref ProjectileInstance projectile, float dt, IProjectileContext context)
        {
            ...
            for (var i = 0; i < count; i++)
            {
                ...
                if (col == null || !col.GetComponentInParent<IDamageable>(out var damageable) || damageable.TeamId == projectile.TeamId)
                {
                    continue;
                }
